            ...
            GeometryReader { proxy in
                TabView {
                    ForEach($tvPageVM.shows, id: \.id) { show in
                        if let posterPath = show.poster_path.wrappedValue {
                            NavigationLink {
                                TVPageDetailView(show: show, detailVM: TVPageDetailViewModel(client: tvPageVM.client, tvID: show.id ?? 0))
                            } label: {
                                ...
                                AsyncImage(
                                    ...
                                    content: { $0.resizable() }, placeholder: { Text("Loading ...") }
                                )
                                .scaledToFill()
                                .cornerRadius(10)
                            }
